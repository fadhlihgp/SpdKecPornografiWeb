...
const SubSidebar = ({link, icon, name}) => {
    ...
    return (
        ...
        >
            <Link to={link} className={"px-3 w-100"} style={{width: "100%", textDecoration: "none", color:"white"}}>
                <img src={icon} width={"20px"} alt={"icon"}/> <span style={{marginLeft: "5%"}}><b>{name}</b></span>
            </Link>
        </div>
    )
}

const Sidebar = ({ role, menus, imageUrl, fullname }) => {
    ...
    return (
        <div className={'m-0'} style={{backgroundColor: "#1B4242", color: "white", width: "17%"}}>
            <div className={"d-flex flex-column align-items-center mt-3 mb-0 text-center"}>
                ...
                <p>{fullname} <br/> <span style={{color: "#D0D4CA"}}>{role}</span></p>
            </div>
            <div>
                {menus.map(({title, value}, index) => (
                    <div className={"mb-3"}>
                        <div style={{fontSize: "14px", color: "#BFCFE7" ,padding: "2% 7%"}}>
                            <b>{title}</b> 
                        </div>
